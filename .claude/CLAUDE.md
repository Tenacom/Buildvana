...
- `tools/lint-docs.cs`: documentation check, the first phase of `inspect.cs`. Its `fenceTags` array is the list in `rules/documentation.md`. Its `todoExemptFiles` array is empty here, and a copy may name the files its TODO check leaves alone. Its `sentenceExemptFiles` array names the two files whose lines still exceed the sentence limit, and a copy may name its own.
