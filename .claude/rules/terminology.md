...
| A `dotnet-tools.json` file, the .NET local tool manifest                                           | tool manifest       | the manifest, local tool manifest                  |
| Running an invocation through the `bv` version that `dotnet-tools.json` pins                       | delegation          |                                                    |
| The `bv` version that `dotnet-tools.json` pins                                                     | pinned `bv`         | the manifest version, the local `bv`               |
