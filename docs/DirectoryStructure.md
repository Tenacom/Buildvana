...
To update the repository as a whole, run `bv update`: it re-pins the repository's entire Buildvana surface — the `bv` entry in the [tool manifest](#configdotnet-toolsjson), the `Buildvana.Sdk` entry in `global.json`, and the [configuration file](BuildvanaToolConfiguration.md)'s `$schema` reference — to the version of the running `bv`, creating files and sections as needed and preserving formatting everywhere. The tool manifest is updated through `dotnet tool update` (or `dotnet tool install --create-manifest-if-needed`), which also downloads the version so the next `dotnet bv` invocation can run it; afterwards, the configuration file is loaded with the new version's model, and any problems are reported as warnings for you to review.
