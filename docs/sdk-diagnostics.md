...
| BVSDK2301 |  Error   | Constant '...' has invalid value '...'.                         | A `ThisAssemblyConstant` item has a `Value` metadata that cannot be parsed according to the [constants syntax](constants-syntax.md): an unknown type prefix, or a value that cannot be parsed as the specified type. |
