...
internal sealed class GitService : IDisposable
{
    ...
    /// <para>Undoing a commit is not an outcome in itself: callers undo a whole set of commits at once, and only
    /// the caller knows what that set was, so the record of the undoing belongs to it. This method only narrates.</para>
    /// </remarks>
    ...
    private bool TryGetOriginInfo([MaybeNullWhen(false)] out string name, [MaybeNullWhen(false)] out string url)
    {
        ...
        var isFirst = true;
        _reporter.Detail("Looking for origin remote...");
        foreach (var remote in _repository.Network.Remotes)
        {
            using (remote)
            {
                _reporter.Detail($"  '{remote.Name}' ({remote.Url})");
        ...
        {
            _reporter.Detail("Origin remote not found.");
            return false;
        }
