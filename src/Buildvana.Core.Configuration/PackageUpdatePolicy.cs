...
/// <summary>
/// The update policy of a package, .NET tool, or MSBuild project SDK pin: how far an automatic update may
/// move the pin, and whether it may land on a prerelease version.
/// </summary>
...
public readonly record struct PackageUpdatePolicy(PackageUpdatePolicyKind Kind, bool AllowPrerelease)
{
    /// <summary>
    /// Parses a policy string: a <see cref="PackageUpdatePolicyKind"/> name, matched case-insensitively,
    ...
    /// here.</para>
    /// </remarks>
    public static bool TryParse(string? text, out PackageUpdatePolicy result)
    {
        if (!UpdatePolicySyntax.TryParse<PackageUpdatePolicyKind>(text, out var kind, out var allowPrerelease))
        {
            result = default;
            return false;
        }

        result = new PackageUpdatePolicy(kind, allowPrerelease);
        return true;
    }
