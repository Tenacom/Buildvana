...
- **BREAKING CHANGE**: `bv` now exits with code 2 when it refuses an invocation, where it used to exit with code 1. A refusal happens before the command runs, and means that the command line names no command bv knows, or gives a command something it does not accept: an unknown command, subcommand, or option; an argument too many or too few; an option value that does not parse. Everything else keeps exit code 1: a command that ran and failed, a configuration file that could not be read or validated, and a repository in a state a command cannot work with. A script that tests for a non-zero exit code needs no change; one that tests for `1` specifically must accept `2` as well. The distinction is worth the break: a caller can now tell "you asked for something impossible" from "what you asked for failed", which the forthcoming `bv dependencies` command relies on.
