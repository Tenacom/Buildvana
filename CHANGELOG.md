...
- `bv dependencies` (alias `bv deps`) is a new command that manages what a repository pins: the .NET SDK version in `global.json`, the MSBuild project SDKs, the .NET local tools, and the NuGet package pins. It has two subcommands. `show`, the default one — so `bv deps` is a complete invocation — works offline, reports what the repository states about itself, and always exits 0 when it completes. `update` resolves target versions against the repository's own package sources and applies them. [Dependency management](docs/DependencyManagement.md) documents the command and the `dependencies` configuration section that drives it, which until now had an effect only through `bv self-update`. The subcommand that removes the pins nothing references any more is being written.
