...
- Buildvana SDK ships the two things `bv dependencies` needs from it: the target that dumps a project's evaluated package items, and the import of the transitive override files that `bv dependencies` will generate once the override lifecycle ships. Neither changes an ordinary build. The dump target runs only when `bv` asks for it, and the override files do not exist until the command that writes them ships. Both are steered by [internal-use properties](docs/InternalUseProperties.md#dependency-management).
