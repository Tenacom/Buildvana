...
- A denied or failed file access during a `bv` command (e.g. a locked or read-only `CHANGELOG.md`, `Directory.Packages.props`, or public API file) no longer surfaces as an unhandled-exception stack trace pointing at `bv` internals. Every file access now reports failure as a single clean error line naming the operation, the path, and the operating-system reason (`Could not read from <path>: <reason>`), and `bv` exits with its regular failure exit code.
