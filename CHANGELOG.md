...
- A `release.emptyChangelog` consisting only of whitespace no longer counts as a substitute for an empty changelog. `bv release` used to announce that it was substituting the configured text, then write a release section with no body at all; it now fails with the same actionable message as when `release.emptyChangelog` is not configured.
