...
- `buildvana.json` accepts a `dependencies` section, describing how far each dependency may move when it is updated and where package pins are declared beyond the files Buildvana finds by itself. It is the configuration surface of the forthcoming `bv deps` command; of what it holds, only `additionalPackages` has an effect today, through `bv self-update` (see below). The section holds:
  - `scopes`: the update policy of each of the four dependency scopes — `netsdk` (the .NET SDK pinned in `global.json`, default `major`), `sdks` (MSBuild project SDKs, default `minor`), `tools` (.NET local tools, default `minor`), and `packages` (NuGet package pins, default `minor`). A policy names the furthest an automatic update may move a version, and each kind of version has a vocabulary of its own. A package pin — the `sdks`, `tools`, and `packages` scopes, and every entry of `policies` — takes `disable`, `exact`, `revision`, `patch`, `minor`, or `major`, one per numeric field a NuGet version may carry, the fourth included. The .NET SDK takes `disable`, `patch`, `feature`, `minor`, `major`, or `lts`: its versions are not SemVer, the feature band being encoded in the patch field, so `patch` names the latest patch within the same feature band and `feature` the latest feature band within the same major and minor, while `lts` moves to the latest version among long-term support releases. A trailing `-` allows prerelease versions (`minor-`). Each position accepts only the policies of its own vocabulary. `lts` and `feature` under `policies` are errors, as are `exact` and `revision` under `scopes.netsdk`. An unparseable policy string is always an error. Buildvana does not fall back to the default, because a silent fallback would hide the typo.
  - `policies`: the update policy of individual packages, written as an object whose member names are patterns matched against a whole package id, with `*` standing for any run of characters. Entries are ranked in document order and the first match wins, so specific patterns go before general ones. They govern the `sdks`, `tools`, and `packages` scopes alike, and outrank the policy of an additional package group.
  - `additionalPackages`: the groups of package pins a repository declares in files of its own, written as an object whose member names are the captions naming the groups in listings. Each group states the glob selecting its files, the MSBuild item name its pins are declared as, and optionally a policy of its own, defaulting to the `packages` scope policy. Pins of a group are updated like any other package pin, but are never pruned and never given transitive overrides.

  The member names of `policies` and `additionalPackages` carry data — a pattern, a caption — so they obey the same rule as any other required string: a blank or all-whitespace name fails validation at configuration load.
