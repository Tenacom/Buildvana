...
- The committed JSON schema (`schemas/buildvana.schema.json`) now carries `examples` annotations, which editors offer as completions. Eighteen model properties state a representative value, covering the settings whose built-in default is absent or unhelpful — a push feed's `source` and `apiKeyEnv`, `git.identity`, an additional package group's `files` and `items`, and the member names of the two keyed objects. Descriptions are shorter to match: each now names its setting in a single line, and the syntax notes fifteen of them used to carry — chiefly "a trailing `-` allows prerelease versions", repeated at twelve policy positions — are documented in this changelog and in [docs/BuildvanaToolConfiguration.md](docs/BuildvanaToolConfiguration.md) instead. Every policy enum still lists `minor-` beside `minor`, so the form stays visible where it is used. A new generated file, [`buildvana.example.jsonc`](buildvana.example.jsonc), states every setting with its description and an example or default value; the repository's own `buildvana.jsonc` no longer doubles as that example.
