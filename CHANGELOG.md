...
- `bv release` now runs an optional repository-owned hook, `.buildvana/hooks/release/post-release.cs`, at the moment the post-release commit is assembled: after the built-in self-reference rewrites (when dogfooding is enabled) and before anything is pushed. The hook is a [file-based app](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/program-structure/file-based-programs) run via `dotnet run` from the home directory; it runs whether or not dogfooding is enabled, and is skipped with an info message when absent. Context is passed through a JSON file whose path is published in the `BV_HOOK_CONTEXT` environment variable; Buildvana SDK injects typed loaders (`BvHookContext`, `BvConfig`) into the compilation of file-based apps under `.buildvana/hooks/`, so hooks need no scaffolding. Files changed by the hook join the post-release commit alongside the built-in rewrites; a non-zero exit code aborts the release before anything is pushed. `bv clean` clears the hooks' file-based-app build caches. See [docs/ReleaseHooks.md](docs/ReleaseHooks.md) for the full contract.
