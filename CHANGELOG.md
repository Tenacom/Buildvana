...
- `buildvana.json` accepts a `git.identity` (`{ name, email }`, both members required whenever the section is stated) section describing the author/committer for automated commits. It is validated and exposed, but not yet wired to release commits.
