...
- `bv dependencies` (alias `bv deps`) is a new command that manages what a repository pins: the .NET SDK version in `global.json`, the MSBuild project SDKs, the .NET local tools, and the NuGet package pins. Its first subcommand, `show`, is also its default one, so `bv deps` is a complete invocation. It works offline, reports what the repository states about itself, and always exits 0 when it completes. [Dependency management](docs/DependencyManagement.md) documents the command and the `dependencies` configuration section that drives it, which until now had an effect only through `bv self-update`. The subcommands that resolve versions against package sources and apply them are being written.
...
- Buildvana SDK ships the two things `bv dependencies` needs from it: the target that dumps a project's evaluated package items, and the import of the transitive override files a forthcoming `bv dependencies update` will generate. Neither changes an ordinary build. The dump target runs only when `bv` asks for it, and the override files do not exist until the command that writes them ships. Both are steered by [internal-use properties](docs/InternalUseProperties.md#dependency-management).
