...
- `buildvana.json` accepts a `dependencies` section, describing how far each dependency may move when it is updated and where package pins are declared beyond the files Buildvana finds by itself. It is the configuration surface of the forthcoming `bv deps` command; of what it holds, only `additionalPackages` has an effect today, through `bv self-update` (see below). The section holds:
  - `scopes`: the update policy of each of the four dependency scopes — `netsdk` (the .NET SDK pinned in `global.json`, default `major`), `sdks` (MSBuild project SDKs, default `minor`), `tools` (.NET local tools, default `minor`), and `packages` (NuGet package pins, default `minor`). A policy names the furthest an automatic update may move a version: `disable`, `exact`, `revision`, `patch`, `minor`, or `major`, plus `lts` for `netsdk` alone, with a trailing `-` allowing prerelease versions (`minor-`). Each position accepts only the policies its own scope has — `lts` under `policies` and `exact` under `scopes.netsdk` are errors — and an unparseable policy string is an error wherever it appears, rather than a silent fall back to the default that would hide the typo.
