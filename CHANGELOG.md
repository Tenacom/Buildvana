...
- The committed JSON schema (`schemas/buildvana.schema.json`) now carries `examples` annotations, which editors offer as completions. Eighteen model properties state a representative value, covering the settings whose built-in default is absent or unhelpful — a push feed's `source` and `apiKeyEnv`, `git.identity`, an additional package group's `files` and `items`, and the member names of the two keyed objects. A member name carries a description of its own as well, which `dependencies.additionalPackages` states. Descriptions are shorter to match: each now names its setting in a single line, and the syntax notes fifteen of them used to carry — chiefly "a trailing `-` allows prerelease versions", repeated at twelve policy positions — are documented in this changelog instead. Every policy enum still lists `minor-` beside `minor`, so the form stays visible where it is used. A new generated file, [`buildvana.example.jsonc`](buildvana.example.jsonc), states every setting with its description and an example or default value; the repository's own `buildvana.jsonc` no longer doubles as that example.
...
- `buildvana.json` accepts a `dependencies` section. The section says how far each dependency may move when it is updated. It also says where package pins are declared, beyond the files Buildvana finds by itself. It is the configuration surface of the forthcoming `bv deps` command. Only `additionalPackages` has an effect today, through `bv self-update` (see below). The section holds:
  - `scopes`: the update policy of each of the four dependency scopes. The scopes are `netsdk` (the .NET SDK pinned in `global.json`), `sdks` (MSBuild project SDKs), `tools` (.NET local tools), and `packages` (NuGet package pins). The default is `major` for `netsdk` and `minor` for the other three. A policy names the furthest an automatic update may move a version. A trailing `-` allows prerelease versions, as in `minor-`. Each kind of version has a vocabulary of its own:
    - a package pin takes `disable`, `exact`, `revision`, `patch`, `minor`, or `major`. Each policy names the finest numeric field it lets move, the fourth field included. The `sdks`, `tools`, and `packages` scopes take this vocabulary, and so does every entry of `policies`;
    - the .NET SDK takes `disable`, `patch`, `feature`, `minor`, `major`, or `lts`. A .NET SDK version is not SemVer, because the feature band is encoded in the patch field. So `patch` names the latest patch within the same feature band, and `feature` the latest feature band within the same major and minor. `lts` moves to the latest long-term support release.

    Each position accepts only the policies of its own vocabulary. `lts` and `feature` under `policies` are errors, as are `exact` and `revision` under `scopes.netsdk`. An unparseable policy string is always an error. Buildvana does not fall back to the default, because a silent fallback would hide the typo.
  - `policies`: the update policy of individual packages. It is an object whose member names are patterns. A pattern is matched against a whole package id, with `*` standing for any run of characters. The entries are ranked in document order, and the first match wins. Put specific patterns before general ones. The entries govern the `sdks`, `tools`, and `packages` scopes alike, and outrank the policy of an additional package group.
  - `additionalPackages`: the groups of package pins a repository declares in files of its own. It is an object whose member names are the captions that name the groups in listings. Each group states the glob selecting its files, and the MSBuild item name its pins are declared as. A group may also state a policy of its own, and takes the `packages` scope policy without one. Pins of a group are updated like any other package pin, and never given transitive overrides. A group's pins are left out of `bv deps prune` as well, unless the group declares `PackageVersion` items: what makes a pin a central pin, and therefore one that can be orphaned, is its item type rather than the file that declares it.

  The member names of `policies` and `additionalPackages` carry data, a pattern or a caption. They obey the same rule as any other required string. A blank or all-whitespace name fails validation at configuration load.
- The `ThisAssemblyClass` SDK module, removed along with code generation tasks after v1.0.0-alpha.20, has been reintroduced. Setting the `GenerateThisAssemblyClass` property to `true` (default: `false`) in a C# project generates a `ThisAssembly` static class containing constants defined via `ThisAssemblyConstant` items, using the syntax documented in [docs/constants-syntax.md](docs/constants-syntax.md). A set of default constants (assembly version, company, product, etc.) is defined unless the `EnableDefaultThisAssemblyConstants` property is set to `false`; the class name and namespace can be customized via the `ThisAssemblyClassName` and `ThisAssemblyClassNamespace` properties. Unlike its previous incarnation, the feature is implemented as a Roslyn incremental source generator, and supports C# projects only: setting `GenerateThisAssemblyClass` to `true` in a project in any other language raises warning BVSDK2300.
...
- `bv dependencies` (alias `bv deps`) is a new command that manages what a repository pins: the .NET SDK version in `global.json`, the MSBuild project SDKs, the .NET local tools, and the NuGet package pins. It has three subcommands. `show`, the default one — so `bv deps` is a complete invocation — works offline, reports what the repository states about itself, and always exits 0 when it completes. `update` resolves target versions against the repository's own package sources and applies them. `prune` removes the central package pins nothing references any more. [Dependency management](docs/tool-commands/dependencies.md) documents the command and the `dependencies` configuration section that drives it, which until now had an effect only through `bv self-update`.
  - The report lists every pin of every selected scope, grouped by the file that declares it, with the update policy governing it. That policy is composed of the pin's own `UpdatePolicy` metadata, the first matching pattern of `dependencies.policies`, the policy of the additional package group the pin belongs to, and the policy of its scope, in that order, which makes `show` the place to see what the ladder produced before anything moves.
  - The report also states what an update could not move, and why: a version range, a floating version, one version written in brackets, a `VersionOverride`, and a version the declaring file states through an MSBuild property rather than as a literal. None of them is an error, because adopting the command must not require rewriting a repository first. A pin stating a prerelease under a policy that takes only stable versions is reported too, as is a `global.json` whose `sdk.allowPrerelease` disagrees with the `netsdk` policy.
  - Two families of options select scopes: `--netsdk`, `--sdks`, `--tools` and `--packages` name the scopes to manage, and `--no-netsdk` and its siblings name the scopes to leave out. The two do not mix, and a scope whose configured policy is `disable` is managed by nothing.
  - Package pins come from MSBuild's own evaluation of the solution, so conditions, imports and layered central package management mean to `bv` what they mean to a build. A repository may declare package pins of its own under `dependencies.additionalPackages`, and a file-based app's `#:package` and `#:sdk` directives are pins like any other. Buildvana's own three packages are invisible to the command in every scope: `bv self-update` is what moves them, all at once.
  - `bv deps update` resolves every pin of every selected scope before it writes anything, so a run either has a target for each pin it manages or changes nothing at all. Versions come from the repository's own package sources, read through NuGet's client libraries — the whole configuration chain, every enabled source type, and package source mapping — and, for the .NET SDK, from the official .NET release index. A pin naming a package or a version no source has is the repository's own error, and one run reports every one of them. A source that cannot answer stops the run, because a resolution against the sources that happened to reply could only be wrong in silence.
  - A pin is moved as far as its policy allows, and never further, never downwards, and never onto a version no source lists. A package pin and a project SDK pin are spliced in the file that declares them, byte for byte outside the version itself; a tool is delegated to `dotnet tool update`, one at a time; and `global.json` is written last of all, since a `global.json` naming an SDK that is not installed breaks every `dotnet` invocation after it. `sdk.allowPrerelease` is written along with it, to say what the `netsdk` policy says.
  - `--check` reports what would change and changes nothing, exiting 1 when anything would: the staleness gate for CI. `--check --all` lists the pins that are already at their target as well, which the report otherwise counts and leaves out. Arguments name the pins a run is about, as package ids or globs; `--to <VERSION>` states the version they must reach, downgrades and frozen pins included, which is the one move no automatic update makes.
  - A new hook, `deps/post-update`, runs at the end of every `bv deps update` that ran to completion, check runs included, and whether or not anything changed: a hook may have derived state to fix even when no pin moved. Its args state what the run made of every pin, and the `global.json` it reads still states the old .NET SDK version while its args state the foreseen one. In a check run, the hook's exit code 1 says that it would change something, and the command folds that into its own verdict; in an apply run, any non-zero exit code is a failure. See [docs/hooks.md](docs/hooks.md).
  - An apply run that manages the `packages` scope ends by lifting the repository's transitive dependencies out of the versions security advisories cover. The versions are forced through generated files that Buildvana SDK imports: `Directory.TransitiveOverrides.props` at the repository root, holding the versions for the projects that manage their package versions centrally, and `<ProjectName>.TransitiveOverrides.props` beside a project, promoting to a reference of that project each package it must lift. Both files are `bv`'s own: they are rewritten from scratch at every apply run, which is also how a stale override disappears, and a promotion carries `PrivateAssets="all"`, so the package a project produces declares exactly the dependencies it would declare without them. The run restores with the files left out of the evaluation to obtain NuGet's verdict on the graph as the repository states it, takes restore's own audit findings — so every NuGet audit setting is honored on its owner's terms, and what a project does not report is not lifted — writes the lowest version each finding calls for that lies outside every advisory and within the package's own update policy, and restores again until the graph stops changing. Only vulnerability lifting is automatic: an override written for it is a lower bound, its oracle is machine-readable, and it disappears once the package that brought the vulnerable version raises its own floor. A vulnerability no version the policy allows would end, and one a decision of the repository's own governs — a vulnerable direct reference, a vulnerable central pin, a central pin below the version a project resolves — are reported as warnings and left alone: `bv` never introduces a version for a package the repository pins or references itself. `bv deps show` lists the overrides in effect offline, and the `deps/post-update` hook is told them. A check run leaves the whole lifecycle alone.
  - `bv deps prune` removes the central package pins nothing references any more. Such pins accumulate on their own: `dotnet package remove` deletes the reference and leaves the pin behind. A pin is orphaned when no project of the solution references its package directly, and no versionless `#:package` directive names it. A project that sets `CentralPackageTransitivePinningEnabled` is judged by its whole resolved graph instead, a pin there raising the version of a package the project never references. The verdict is NuGet's own: the command restores the solution — with the transitive override files left out of the evaluation, so a promotion `bv` wrote never makes a pin look alive — and reads the direct references each project's dependency graph states, which is how a reference written through a property or an item transform counts like any other. That restore is why the diagnosis is not part of the offline `show`. Only central pins can be orphaned: a `PackageReference` is itself the reference, as is a `#:package` directive carrying a version, and the `netsdk`, `sdks` and `tools` scopes are ignored in silence. Policy plays no part, a policy saying how far a pin may move and nothing about whether the repository still needs it, so a pin whose policy is `disable` is removed like any other. `prune --check` reports the orphaned pins and removes none, exiting 1 when there is at least one; no argument names the pins a run is about, because a filter that hid an orphan would leave the repository stating a pin the same run had just called dead. A removal takes the item element out of the file that declares it, with its line, and leaves everything else — comments, formatting, an item group left with no items — byte for byte as it was. An apply run regenerates the transitive override files before it ends, whether or not it removed anything, since a promotion may rest on a pin that has just gone. A check run writes none of them, and restores once more with them in the evaluation, so that no build is left with the graph the diagnosis produced. Every `prune` that managed the `packages` scope and ran to completion runs the `deps/post-update` hook, whether or not anything was removed, with every pin stated as skipped and a removed pin not stated at all.
- `bv dependencies update --latest` moves the pins a run is about [past their policy](docs/tool-commands/dependencies.md#moving-a-pin-past-its-policy), to the latest version the sources have.
- Buildvana SDK ships the two things `bv dependencies` needs from it: the target that dumps a project's evaluated package items, and the import of the transitive override files. Neither changes an ordinary build. The dump target runs only when `bv` asks for it, and a repository whose graph needs no override has no such file to import. Both are steered by [internal-use properties](docs/internal-use-properties.md#dependency-management).
...
- **BREAKING CHANGE**: `bv`'s exit codes are now one vocabulary, and every command uses it. `0` means the command completed. `1` means the command ran and failed: a repository in a state it cannot work with, a configuration file that could not be read or validated, a file that could not be read or written. `2` means `bv` refused the command line — an unknown command, subcommand, or option; an argument too many or too few; an option value that does not parse — where it used to exit with `1`. `3` means that a program `bv` invoked failed, or produced output `bv` cannot use; `bv` used to return that program's own exit code as its own, so a failing `dotnet build` made `bv build` exit with `1`, and a hook exiting with `2` made `bv release` look like a usage error. The program's own exit code is now reported in the message instead, because its meaning belongs to whoever wrote the program. `130` still means the run was terminated with Ctrl-C. A delegated run still returns the pinned `bv`'s exit code as it stands: the delegated `bv` is the one that ran the command. A script that tests for a non-zero exit code needs no change; one that tests for `1` specifically must accept `2` and `3` as well. The distinction is worth the break: a caller can now tell "you asked for something impossible" from "what you asked for failed" and from "the toolchain underneath failed". See [Diagnostics and exit codes of `bv`](docs/tool-diagnostics.md#exit-codes).
