...
- The configuration file is now also recognized in a `.buildvana` directory directly under the home directory (`.buildvana/buildvana.json` or `.buildvana/buildvana.jsonc`), so that repositories accumulating more Buildvana-related files can group them there instead of cluttering the root. Exactly one configuration file may exist per home directory across the four candidate locations; any two coexisting is an error naming all offending files, generalizing the previous rule against having both `buildvana.json` and `buildvana.jsonc`. A configuration file inside `.buildvana` is a home-directory marker like one in the root, marking the directory that contains `.buildvana` as home; a `.buildvana` directory without a configuration file is not a marker.
...
- `bv release` now runs an optional repository-owned hook, `.buildvana/hooks/release/post-release.cs`, at the moment the post-release commit is assembled: after the built-in self-reference rewrites (when dogfooding is enabled) and before anything is pushed. The hook is a [file-based app](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/program-structure/file-based-programs) run via `dotnet run` from the home directory; it runs whether or not dogfooding is enabled, and is skipped with an info message when absent. Context is passed through a JSON file at a per-hook well-known path, `.buildvana-temp/hook-contexts/release/post-release.json`, (re)written before each hook run and left in place afterwards, so a hook can be re-run by hand against the context of the last run; `.buildvana-temp/` is bv's scratch directory for machine-generated temporary files, recommended for gitignoring and always excluded from bv's own working-tree change detection. Files changed by the hook join the post-release commit alongside the built-in rewrites; a non-zero exit code aborts the release before anything is pushed. `bv clean` clears the hooks' file-based-app build caches and deletes the scratch directory. See [docs/ReleaseHooks.md](docs/ReleaseHooks.md) for the full contract.
- A new packaged library, `Buildvana.Runtime`, holds the typed model of the Buildvana configuration file and of the run-time information `bv` shares with repository-owned hooks: `BuildvanaConfig` (with `BuildvanaConfig.Load()`, a strict loader usable from file-based apps, where reflection-based JSON serialization is disabled), `PostReleaseHookContext` (with `PostReleaseHookContext.Load()`, structured into `Paths` and `Release` sections plus hook-specific members), and the well-known path constants shared by both sides of the hook contract. A hook references the package with an unversioned `#:package Buildvana.Runtime` directive; Buildvana SDK pins the package to its own version for every file-based app built in the repository, so `bv`, the SDK, and hooks always agree on the shape of the data.
...
- A new `bv sync-sdk` command aligns the pinned Buildvana SDK version with the running `bv`'s version, updating whichever is older. When the pin is older, missing, or invalid, the `global.json` pin is rewritten to `bv`'s version, creating the file and/or the `msbuild-sdks` section if needed and preserving the file's formatting otherwise. When the pin is newer, `bv` itself is updated via `dotnet tool update` — but only when the running `bv`'s version matches the `bv` entry in the repository's tool manifest (`.config/dotnet-tools.json`): version equality is how `sync-sdk` decides that the running `bv` is the manifest's, so a `bv` installed some other way (e.g. globally, or run via `dnx`) can pass the guard only when its version coincides with the manifest's entry, in which case the update is harmless — the manifest ends up pinning exactly the version `global.json` asks for; when the versions differ, or there is no manifest entry, the command fails with instructions instead. When the versions already match, the command changes nothing. Whenever the pin ends up matching the running `bv` but the tool manifest still pins a different `bv` version, `sync-sdk` warns — visibly at default verbosity — that the next `dotnet bv` invocation will run the manifest's version and fail the SDK version check, and suggests how to complete the alignment.
...
- **BREAKING CHANGE**: The `.buildvana-home` marker file is no longer recognized: home-directory discovery now only looks for a Buildvana configuration file (in the home directory root or in its `.buildvana` subdirectory) and Git markers. `.buildvana-home` predates the configuration file, and a `buildvana.json` containing an empty object (`{}`) does the same job — marking a directory as home without configuring anything — while being what one would naturally reach for. To migrate, replace `.buildvana-home` with a `buildvana.json` file containing `{}`.
...
- `bv` commands that take no options of their own (`clean`, `sync-sdk`, `version show`) no longer silently ignore unknown options: `bv clean --bogus` now fails with `Unknown option '--bogus' for command 'clean'`. Commands with their own options already rejected unknown ones.
