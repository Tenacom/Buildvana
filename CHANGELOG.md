...
- A new `bv sync-sdk` command aligns the pinned Buildvana SDK version with the running `bv`'s version, updating whichever is older. When the pin is older, missing, or invalid, the `global.json` pin is rewritten to `bv`'s version, creating the file and/or the `msbuild-sdks` section if needed and preserving the file's formatting otherwise. When the pin is newer, `bv` itself is updated via `dotnet tool update` — but only when the running `bv` comes from the repository's tool manifest (`.config/dotnet-tools.json`); a `bv` installed some other way (e.g. globally, or run via `dnx`) is never updated behind the manifest's back, and the command fails with instructions instead. When the versions already match, the command changes nothing.
